...
def clean_session_manager(mock_session_file):
    ...
    with patch(
        "ac_cdd_core.session_manager.SessionManager.SESSION_FILE", mock_session_file
    ):
        yield
...
async def test_resume_jules_session_auto_load(clean_session_manager):
    ...
    with patch(
        "ac_cdd_core.services.jules_client.JulesClient",
        return_value=mock_jules_instance,
    ):
        ...
        mock_jules_instance.wait_for_completion.assert_called_once_with(
...
async def test_resume_jules_session_no_id_found(tmp_path):
    ...
    with patch("ac_cdd_core.session_manager.SessionManager.SESSION_FILE", empty_file):
        with pytest.raises(
            SessionValidationError, match="No Jules Session ID provided or found"
        ):
