...
def test_config_update():
    ...
    assert config.sandbox.install_cmd == expected_cmd, f"Expected {expected_cmd}, got {config.sandbox.install_cmd}"
...
async def test_syntax_check_node(mock_services):
    ...
    with patch.object(builder, '_get_shared_sandbox', return_value=mock_runner):
        ...
        # (plus the dependency check if _get_shared_sandbox logic triggers it, but we mocked the runner not the method entirely?
        # Wait, I patched _get_shared_sandbox to return mock_runner directly, so the logic inside _get_shared_sandbox regarding ruff version check is SKIPPED by the patch.
